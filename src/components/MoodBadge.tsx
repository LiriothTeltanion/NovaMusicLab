...
export default function MoodBadge({ moodKey, confidence, size = 'md', className = '' }: MoodBadgeProps) {
  ...
  const description = pickLanguage(lang, mood.description);
  const shortLabel = pickLanguage(lang, mood.shortLabel);

  return (
    <span
      className={`nova-bidi-isolate inline-flex items-center gap-1.5 rounded-full font-mono font-black uppercase tracking-widest ${compact ? 'px-2 py-0.5 text-[9px]' : 'px-2.5 py-1 text-[10px]'} ${className}`}
      style={{
        color: mood.color,
