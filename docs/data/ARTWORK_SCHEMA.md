...
- `scripts/capture_readme_visuals.mjs` is the canonical generator for the 1280×640 Open Graph/Twitter PNG. `assets/social/nova-music-lab-social-preview.svg` is an editable design reference whose copy should remain aligned, but it is not rasterized by the release pipeline. `public/social-preview-v2.png` remains the reviewed artifact for `1.5.0` — deployed 2026-08-01 — while `public/social-preview.png` is preserved as the legacy fallback. The `v1.6.0` private candidate updates the generator and SVG source from the public manifest and share metrics; its final PNG is intentionally pending the frozen-source capture and media audit.
